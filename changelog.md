...
- `construction`: a demolished office was still written to, because the plugin kept the pointer it had found and never asked again. Every write now re-checks that the address is still readable, still a construction office of the type it matched, and not already flagged as going away, and drops it until the building list hands it back. `MAX_OFFICES` went from 16 to 64 on the evidence of a map with 17 of them, whose seventeenth office silently never got its range set. The log also now says, once per office, what the range was and what it became - the absence of that line is how an argument about the game's own default ended up being settled by git archaeology. It is 1000, and it took a fresh map to say so: both of the plugin's writes switched off, a brand-new map, one freshly built office, and its window read "<1,000m" before anything had touched it. The 3500 that sat in the ini before it had two pieces of evidence behind it and that is exactly why it was convincing - but the sixteen probed offices that all read 3500 came out of a save whose offices had every one of them been ridden to the `+` button's top by hand, and the executable's own `mov [rsi+0xFC8],3500` is that button's clamp rather than the value an office is born with. Both readings were right about the offset, which was what they were actually measuring. 1000 had also sat there earlier as a pure guess at one rung of the `+`/`−` button ladder - the one the handler computes at run time instead of carrying as an immediate, so the disassembly does not name it - and it is not that rung; it was the right number for the wrong reason.
